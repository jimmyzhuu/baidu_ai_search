"""Smart search tool for Baidu AI Search."""
...
from tools._response import build_smart_search_payload, normalize_smart_search_output

SMART_SEARCH_API_URL = "https://qianfan.baidubce.com/v2/ai_search/chat/completions"
...
class SmartSearchTool(Tool):
    def _invoke(self, tool_parameters: dict[str, Any]) -> Generator[ToolInvokeMessage]:
        ...
        payload = build_smart_search_payload(query, tool_parameters.get("top_k"))

        try:
            response = requests.post(SMART_SEARCH_API_URL, headers=headers, json=payload, timeout=REQUEST_TIMEOUT)
        ...
        yield self.create_json_message(normalize_smart_search_output(response.json()))
